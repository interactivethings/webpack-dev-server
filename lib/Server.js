...
function Server(compiler, options) {
	...
	if(options.contentBase !== false) {
		...
		if(typeof contentBase === "object") {
			...
			var defaultMiddleware = ["*", this.setContentHeaders.bind(this), this.serveMagicHtml.bind(this), express.static(contentBase)];
			if (options.serveIndex !== false) {
				defaultMiddleware.push(serveIndex(contentBase));
			}
			app.get.apply(app, defaultMiddleware);
		}
	}
